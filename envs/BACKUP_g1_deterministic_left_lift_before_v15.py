...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _recovery_env(self, phi: float) -> float:
        ...
        Stage 1:
            touchdown -> establish stable two-foot force sharing

        Stage 2:
            after stable load sharing -> slowly return posture to stand

        The recovery timer therefore does NOT start at touchdown.
        """
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        if (
            ...
            if not self._recovery_load_ready:
                ...
                # ------------------------------------------------------
                # Small deadband:
                # do not aggressively chase tiny force differences.
                # ------------------------------------------------------

                if abs(load_error) < 0.04:
                ...
                # Same successful V12 transfer direction.
                #
                # Slightly reduced amplitude because we now only need
                # to reach the first valid shared-support state.
                # V14:
                # Restore the stronger V12 transfer authority.
                #
                # V12 experimentally reached Lload ~= 0.39.
                # V13 weakened these values and never got past
                # approximately Lload=0.07.
                #
                # The V13 immediate latch now prevents the old V12
                # overshoot after shared support is reached.
            ...
            # AFTER loadOK:
            #
            # NO strong force transfer.
            #
            # Recovery starts from the actual captured two-foot pose.
            # ----------------------------------------------------------
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v14_strong_transfer_with_latch",
        }
