...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _recovery_env(self, phi: float) -> float:
        """
        V15 shared-support recovery.

        Recovery begins ONLY after loadOK.

        After shared support is captured:
            1. hold it briefly,
            2. then slowly recover toward stand.

        Closed-loop foot-load feedback remains active separately.
        """
        ...
        elapsed = max(
            ...
            (
                ...
                - float(self._recovery_load_ready_step)
            )
            * dt_control,
        )

        # First allow the freshly captured two-foot state to settle.
        recovery_dwell = 0.30

        if elapsed <= recovery_dwell:
            return 0.0
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        if (
            ...
            # ----------------------------------------------------------
            # V15 CONTINUOUS FORCE-SHARING FEEDBACK
            # ----------------------------------------------------------
            ...
            if self._recovery_load_ready:
                ...
                # Start from the ACTUAL successfully captured load share.
                #
                # As recovery progresses, slowly approach the approximately
                # even load distribution seen in the initial stable stand.
                ...
                # Moderate stabilizing authority after loadOK.
                feedback_scale = 0.85
                deadband = 0.025

            else:

                # Before loadOK, continue using the stronger transfer needed
                # to FIND the first valid shared-support state.
            ...
            if abs(load_error) <= deadband:
                ...
                # Remove the deadband discontinuity before scaling.
                effective_error = (
                    load_error
                    - np.sign(load_error)
                    * deadband
            ...
            # ----------------------------------------------------------
            # SAME experimentally verified transfer direction as V12/V14.
            #
            # These commands drove:
            #
            #     Lload ~= 0.01
            #          ->
            #     Lload ~= 0.39
            #
            # in the successful V14 run.
            # ----------------------------------------------------------
            ...
            # AFTER loadOK:
            #
            # V15 keeps moderate closed-loop force sharing active.
            #
            # Recovery still starts from the actual captured two-foot pose.
            # ----------------------------------------------------------
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v15_closed_loop_shared_recovery",
        }
