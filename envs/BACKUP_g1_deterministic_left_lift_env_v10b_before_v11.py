...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _update_touchdown_state(self) -> None:
        """
        Detect real LEFT-foot touchdown.

        The left foot first has to enter a genuine swing state.
        Only a later ground contact can count as touchdown.
        """
        if not self._lift_enabled:
            return
        ...
        left_clearance = max(
            0.0,
            left_z
            - float(self.left_foot_p0[2]),
        )

        # --------------------------------------------------------------
        # First prove that LEFT actually entered swing.
        # --------------------------------------------------------------
        if (
            (not left_contact)
            or left_clearance >= 0.010
        ):
            self._left_was_airborne = True

        # --------------------------------------------------------------
        # Then detect the FIRST genuine physical re-contact.
        #
        # V9 log had the ideal event near:
        #     Lcontact=1
        #     LF ~= 29 N
        #     clearance ~= 6 mm
        # --------------------------------------------------------------
        ...
        if touchdown_ok:
            ...
            # Actual physical posture at touchdown.
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        # --------------------------------------------------------------
        # V10B FINAL TOUCHDOWN RECOVERY OVERRIDE
        # --------------------------------------------------------------
        #
        # Everything above is useful during preparation and swing.
        #
        # Once touchdown occurs, however, do NOT allow the old
        # single-support controllers to keep commanding the robot.
        #
        # Start exactly from the measured physical touchdown posture and
        # smoothly blend toward the known stable two-foot stand posture.
        if (
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v10b_touchdown_recovery",
        }
