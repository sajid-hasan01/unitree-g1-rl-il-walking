...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _recovery_env(self, phi: float) -> float:
        ...
        Recovery does NOT begin from global phase anymore.

        It begins only after:
            1. LEFT foot has genuinely entered swing,
            2. LEFT foot physically contacts the floor again,
            3. measurable ground force is present.

        This prevents the V9 failure where recovery began while the
        left foot was still airborne.
        """
        ...
        elapsed = max(
            ...
            (
                ...
                - float(self._touchdown_step)
            )
            * dt_control,
        )

        # Deliberately slow recovery.
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        if self._support_ready_latched:
            ...
            # Full conservative support authority while left foot swings.
            #
            # Once recovery begins, progressively release the
            # right-foot-only support lock.
            support_env = (
                0.40
                * (1.0 - recovery)
            )
        ...
        if (
            ...
            if self._emergency_landing:
                ...
                # sw is already approaching/at zero here.
                #
                # Therefore desired foot position is left_foot_p0:
                # the original floor position.
                #
                # Keep it pinned while pelvis/COM moves back to center.
                blend = (
                    0.80
                    * (1.0 - recovery)
                    + 0.30
        ...
        # --------------------------------------------------------------
        # V11 TOUCHDOWN RECOVERY OVERRIDE
        # --------------------------------------------------------------
        #
        # V10B replaced ALL 15 joint commands here.
        #
        # That erased the successful sagittal x/xv stabilizer.
        #
        # V11 only blends LATERAL roll joints back to standing.
        #
        # Sagittal hip/knee/ankle/waist-pitch commands remain controlled
        # by the already successful V5 sagittal feedback above.
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v11_confirmed_air_and_sagittal_recovery",
        }
