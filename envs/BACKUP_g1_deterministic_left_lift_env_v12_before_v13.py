...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _update_recovery_load_state(self) -> None:
        ...
        After LEFT touchdown, do not immediately begin posture recovery.

        First require both feet to support the robot with a sensible
        load distribution.
        ...
        stable_load = (
            ...
            and left_load_ratio >= 0.32
            and left_load_ratio <= 0.62
        ...
        # Require 15 consecutive stable control samples.
        if self._recovery_load_streak >= 15:
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        # --------------------------------------------------------------
        # V12 FORCE-CONTROLLED TWO-FOOT RECOVERY
        # --------------------------------------------------------------
        #
        # V11 blended lateral joint angles toward stand without checking
        # ground forces.
        #
        # Result:
        #     Lload 0.29 -> 0.05 -> 0.00
        #
        # V12 uses the measured LEFT/RIGHT force distribution as feedback.
        if (
            ...
            # ----------------------------------------------------------
            # Base lateral posture.
            #
            # Before force sharing is stable:
            #     hold actual touchdown posture.
            #
            # After loadOK:
            #     slowly blend toward normal stand.
            # ----------------------------------------------------------
            recovery_pose = (
                (1.0 - recovery)
                * self._touchdown_q
            ...
            # Aim near balanced two-foot support.
            ...
            # Positive command:
            #     LEFT needs more load
            #     -> move body away from RIGHT support side.
            load_cmd = float(
                np.clip(
                    load_error / 0.30,
                    -1.0,
            ...
            # These signs are exactly the OPPOSITE of the known
            # rightward weight-shift pattern used during preparation.
            #
            # Therefore positive load_cmd moves weight back toward LEFT.
            force_gain = (
                1.0
                - 0.40 * recovery
            )
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v12_force_shared_recovery",
        }
