...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        sagittal_scale = 1.0 + 0.65 * sw

        # V4 SIGN FIX:
        # In the actual runs, negative x / negative xv produced positive xcorr.
        # The previous mapping then commanded +right-hip-pitch and
        # -right-ankle-pitch, while the pelvis accelerated even farther in
        # negative x. Reverse the stance sagittal mapping so the feedback acts
        # against the measured backward motion rather than reinforcing it.
        off[6] += -0.90 * sagittal_scale * xcorr      # right hip pitch
        off[10] += +0.70 * sagittal_scale * xcorr     # right ankle pitch
        off[14] += +0.28 * sagittal_scale * xcorr     # waist pitch
        ...
        off[9] += +0.030 * support_env + 0.42 * height_task
        off[6] += +0.16 * height_task
        off[10] += -0.14 * height_task

        # --------------------------------------------------------------
        # D) LEFT swing feedforward. Only active AFTER support gate passes.
        # --------------------------------------------------------------
        off[0] += +0.010 * sw
        off[3] += +0.26 * sw
        off[4] += +0.14 * sw
        # No fixed left-hip-roll swing bias in V3. The lateral balance
        # controller owns roll while the task-space IK owns foot height.
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v4_sagittal_sign_and_early_land",
        }
