...
class DeterministicLeftLiftConfig:
    ...
    lift_start: float = 0.44
    lift_full: float = 0.515
    hold_end: float = 0.545
    lower_end: float = 0.635
    shift_return_end: float = 0.78
    ...
    com_shift_fraction: float = 0.58
    support_gate_start: float = 0.26
    com_gate_tolerance: float = 0.020
...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _maybe_enable_lift(self) -> None:
        ...
        if not self._support_ready_latched:
            ...
            right_contact = self._foot_contact(self.right_foot_body)
            up_z = self._root_up_z()

            com_ok = com_error <= c.com_gate_tolerance
            contact_ok = bool(right_contact)
            upright_ok = up_z >= c.gate_min_up_z

            if not contact_ok:
                self._gate_fail_reason = "right_support_contact_missing"
            elif not upright_ok:
                self._gate_fail_reason = "upright_gate_failed"
            elif not com_ok:
                self._gate_fail_reason = "com_transfer_incomplete"
            else:
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        if sw > 0.001:
            ...
            swing_dofs = [self.qvel_adrs[i] for i in swing_idx]

            current = self.data.site_xpos[self.left_foot_site].copy()
            desired = self.left_foot_p0.copy()
            desired[2] += self.cfg.target_clearance * sw
        ...
        # --------------------------------------------------------------
        # V6: LEFT-FOOT CONTACT RELEASE SEED
        # --------------------------------------------------------------
        #
        # V5 proved that whole-body balance is stable, but pure task-space
        # IK cannot reliably initiate swing from the loaded two-foot-contact
        # posture.
        #
        # Add a SMALL temporary knee/ankle flexion only while the left foot
        # still needs help breaking contact.
        #
        # Once clearance approaches 1 cm, this seed fades out and task-space
        # IK controls the rest of the lift.
        if sw > 0.02:
            left_contact_now = bool(info["left_contact"])
            left_clear_now = float(info["left_foot_clearance"])

            if left_contact_now:
                release_need = 1.0
            else:
                release_need = float(np.clip(
                    (0.010 - left_clear_now) / 0.010,
                    0.0,
                    1.0,
                ))

            # Smoothly introduce the release instead of kicking the leg.
            release_ramp = float(np.clip(sw / 0.35, 0.0, 1.0))
            release = release_need * release_ramp

            # Much smaller than the old swing feedforward.
            target[0] += +0.008 * release     # left hip pitch
            target[3] += +0.120 * release     # left knee flexion
            target[4] += +0.060 * release     # left ankle pitch
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v6_contact_release",
        }
