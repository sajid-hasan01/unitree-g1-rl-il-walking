...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _recovery_env(self, phi: float) -> float:
        ...
        V8 successfully lifted/unloaded the left foot, but after
        lowering it continued holding the latched right-support
        posture forever.

        Recovery starts after lower_end and smoothly:
            support pose -> normal stand pose
            support COM  -> original neutral COM
        """
        if not self._lift_enabled:
            return 0.0

        if phi <= self.cfg.lower_end:
            return 0.0

        if phi >= self.cfg.shift_return_end:
            return 1.0

        u = (
            (phi - self.cfg.lower_end)
            /
            max(
                self.cfg.shift_return_end
                - self.cfg.lower_end,
                1e-6,
            )
        )
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_diagnostic_v9_recovery_blend",
        }
