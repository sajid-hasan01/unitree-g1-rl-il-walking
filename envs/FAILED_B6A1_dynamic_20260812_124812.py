...
class DeterministicLeftLiftConfig:
    ...
    # V8:
    # V7 showed an excellent support state around phi ~= 0.345.
    # Do not continue waiting and leaning after that.
    lift_start: float = 0.40
    lift_full: float = 0.48
    hold_end: float = 0.54
    lower_end: float = 0.64
    shift_return_end: float = 0.76
    ...
    #   x     ~= 0
    #
    # was already an excellent preparation state.
    #
    # Do NOT wait until the left load reaches almost zero by leaning
    # the entire robot farther sideways.
    com_shift_fraction: float = 0.70
    support_gate_start: float = 0.26
    com_gate_tolerance: float = 0.015
    gate_min_up_z: float = 0.94

    # 25-28% left-foot load is sufficiently unloaded for the small
    # contact-release motion. The swing itself will finish unloading it.
    max_left_load_ratio: float = 0.28

    # Force-gated release.
    release_load_ratio_full: float = 0.28
    release_load_ratio_zero: float = 0.05
    ...
    # ----------------------------------------------------------
    # B6A LINE-FOLLOWING HIP-PITCH OVERLAY
    #
    # B6-SENS / HIP-SCAN showed that negative LEFT hip pitch:
    #   - increases true physical sole clearance
    #   - moves the swing foot forward
    #   - causes negligible lateral displacement
    #
    # B6A-1 deliberately starts at only -1.0 deg dynamically.
    # ----------------------------------------------------------
    b6a_hip_pitch_enabled: bool = True
    b6a_hip_pitch_deg: float = 1.00
    b6a_hip_fade_start: float = 0.60
    b6a_hip_fade_end: float = 0.90
...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        # --------------------------------------------------------------
        # V10B physical touchdown state
        # --------------------------------------------------------------
        self._left_was_airborne = False
        self._touchdown_latched = False
        self._touchdown_step = -1
        self._touchdown_q = None
        self._touchdown_com_y = 0.0

        # V11:
        # Require sustained force-free air before touchdown.
        self._left_air_streak = 0
        self._left_max_air_streak = 0
        self._touchdown_contact_streak = 0
        self._left_peak_clearance = 0.0
        self._swing_confirmed = False

        # --------------------------------------------------------------
        # V12 two-foot force-sharing recovery
        # --------------------------------------------------------------
        self._recovery_load_streak = 0
        self._recovery_load_ready = False
        self._recovery_load_ready_step = -1

        # V13:
        # actual mechanically-valid two-foot support state
        self._shared_support_q = None
        self._shared_support_com_y = 0.0
        self._shared_support_left_load = 0.0

        # V15 recovery diagnostics.
        self._recovery_desired_left_load = 0.0
        self._recovery_load_cmd = 0.0

        # V17:
        # closed-loop COM hold around captured shared-support state.
        self._shared_hold_com_error = 0.0
        self._shared_hold_com_cmd = 0.0
    ...
    def _recovery_env(self, phi: float) -> float:
        """
        V16 VALIDATION HOLD.

        Once a physically valid two-foot support state is captured,
        hold that measured state for the remainder of the episode.

        No neutral-centering is attempted here.

        This isolates and validates:

            prepare
              -> lift
              -> airborne
              -> touchdown
              -> shared two-foot support
              -> stable settle
        """
        return 0.0
    ...
    def _swing_env(self, phi: float) -> float:
        ...
        # Once physical touchdown is captured, the old swing trajectory
        # is permanently disabled for this episode.
        if self._touchdown_latched:
            return 0.0
    ...
    def _update_touchdown_state(self) -> None:
        # V11 robust swing/touchdown detector.
        #
        # A brief contact break is NOT enough to declare the leg airborne.
        # We first require a sustained force-free airborne phase.
        ...
        self._left_peak_clearance = max(
            float(
                self._left_peak_clearance
            ),
            float(
                left_clearance
            ),
        ...
        if air_now:
            ...
            self._left_air_streak += 1

        else:

            if not self._swing_confirmed:
                self._left_air_streak = 0

        self._left_max_air_streak = max(
            int(
                self._left_max_air_streak
            ),
            int(
                self._left_air_streak
            ),
        )

        # Require FIVE consecutive control samples
        # completely free of LEFT ground force.
        if self._left_air_streak >= 5:

            self._swing_confirmed = True
            self._left_was_airborne = True

        # ----------------------------------------------------------
        # B) TOUCHDOWN IS IMPOSSIBLE UNTIL AIR IS CONFIRMED
        # ----------------------------------------------------------
        ...
        touchdown_now = (
            left_contact
            and left_force >= 18.0
            and left_clearance <= 0.012
        )

        if touchdown_now:

            self._touchdown_contact_streak += 1

        else:

            self._touchdown_contact_streak = 0

        # Require 3 consecutive touchdown samples.
        if self._touchdown_contact_streak < 3:
            return

        # ----------------------------------------------------------
        # C) LATCH REAL TOUCHDOWN
        # ----------------------------------------------------------

        self._touchdown_latched = True

        self._touchdown_step = int(
            self.episode_step
        )

        self._touchdown_q = np.array(
            [
                self.data.qpos[
                    self.qpos_adrs[i]
                ]
        ...
        self._touchdown_com_y = float(
            self._whole_body_com()[1]
        )



    def _update_recovery_load_state(self) -> None:
        """
        V13 shared-support latch.

        V12 proved that a valid two-foot state occurs naturally after
        touchdown.
        ...
        That state must be captured immediately.

        Do NOT continue pushing lateral force transfer after reaching it.
        """
        if not self._touchdown_latched:

            self._recovery_load_streak = 0
            return

        if self._recovery_load_ready:
            return
        ...
        stable_load = (
            ...
            and right_contact
            and left_force >= 25.0
            and right_force >= 25.0
            # V16:
            # latch earlier so we stop lateral transfer BEFORE overshoot.
            and left_load_ratio >= 0.25
            and left_load_ratio <= 0.60
            and self._root_up_z() >= 0.94
        )
        ...
        # ----------------------------------------------------------
        # FIRST valid shared-support sample = latch immediately.
        # ----------------------------------------------------------

        self._recovery_load_streak = 1

        self._recovery_load_ready = True

        self._recovery_load_ready_step = int(
            self.episode_step
        )

        self._shared_support_q = np.array(
            [
                self.data.qpos[
                    self.qpos_adrs[i]
                ]
        ...
        self._shared_support_com_y = float(
            self._whole_body_com()[1]
        )

        self._shared_support_left_load = float(
            left_load_ratio
        )
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        recovery = self._recovery_env(phi)

        if self._support_ready_latched and self._support_ready_q is not None:

            # During lift:
            #     hold the good right-support posture.
            #
            # After lowering:
            #     smoothly return to normal two-foot standing posture.
            target = (
                (1.0 - recovery)
                * self._support_ready_q
                +
                recovery
        ...
        # V9:
        # During single support use right-support COM target.
        # During recovery move the target back to original standing COM.
        if (
            self._recovery_load_ready
            and self._shared_support_q is not None
        ):

            desired_com_y = float(
                (1.0 - recovery)
                * self._shared_support_com_y
                +
                recovery
                * self.com_p0[1]
            )

        elif self._touchdown_latched:

            desired_com_y = float(
                self._touchdown_com_y
            )
        ...
        if self._support_ready_latched:

            if self._touchdown_latched:

                # After LEFT touchdown the robot is no longer supposed
                # to behave as a pure RIGHT-single-support system.
                #
                # Keep only a light right-foot lock while force sharing
                # is established, then fade it further during recovery.
                support_env = (
                    0.20
                    * (1.0 - recovery)
                )
                ...
                # During actual LEFT swing:
                # preserve the successful V5/V11 support authority.
                support_env = 0.40
        ...
        # V16:
        # once true two-foot support is captured, the robot is no longer
        # a RIGHT-single-support system.
        if self._recovery_load_ready:
            support_env = 0.0
        ...
        if (
            sw > 0.001
            or self._emergency_landing
            or self._touchdown_latched
            or recovery > 0.0
        ):
            ...
            if self._emergency_landing:
                ...
                # V12:
                # Pin LEFT foot near its original floor position after
                # touchdown.
                #
                # IMPORTANT:
                # The V9/V11 expression could produce blend > 1.0.
                # Keep the blend explicitly bounded.
                blend = float(
                    np.clip(
                        0.85
                        - 0.35 * recovery,
        ...
        if (
            ...
            and not self._touchdown_latched
        ):
            (
                left_force_now,
                right_force_now,
            ...
            # V8:
            # Only enough motion to break the remaining light contact.
            #
            # Task-space IK remains responsible for the real lift.
            target[0] += (
                +0.002 * release
            )                       # left hip pitch
        ...
        # --------------------------------------------------------------
        #
        # Negative LEFT hip pitch was measured to produce BOTH:
        #
        #   increased true sole clearance
        #   +
        #   forward swing-foot displacement
        #
        # Only target[0] is changed.
        #
        # No ankle / knee / support / COM / touchdown /
        # shared-support / recovery law is modified.
        #
        _b6a_hip_trim = 0.0

        if (
            self.cfg.b6a_hip_pitch_enabled
            and sw > self.cfg.b6a_hip_fade_start
        ):
            _b6a_u = float(
                np.clip(
                    (
                        sw
                        - self.cfg.b6a_hip_fade_start
                    )
                    / max(
                        self.cfg.b6a_hip_fade_end
                        - self.cfg.b6a_hip_fade_start,
                        1e-9,
                    ),
                    0.0,
            ...
            _b6a_fade = self._smoothstep(
                _b6a_u
            )

            _b6a_hip_trim = (
                -math.radians(
                    self.cfg.b6a_hip_pitch_deg
                )
                * _b6a_fade
            )

            target[0] += _b6a_hip_trim

        # V13 SHARED-SUPPORT-LATCH RECOVERY
        # --------------------------------------------------------------
        #
        # Stage A:
        #     touchdown but not yet balanced
        #     -> use force feedback to FIND two-foot support
        #
        # Stage B:
        #     first valid two-foot support
        #     -> latch actual posture
        #     -> STOP strong force feedback
        #
        # Stage C:
        #     slowly blend captured shared-support pose toward stand
        if (
            self._touchdown_latched
            and self._touchdown_q is not None
        ):

            recovery = self._recovery_env(
                phi
            )

            # ----------------------------------------------------------
            # Select the physically correct recovery starting posture.
            # ----------------------------------------------------------
            ...
            for joint_i in lateral_recovery_idx:
            ...
            # ----------------------------------------------------------
            # FORCE FEEDBACK IS ALLOWED ONLY UNTIL SHARED SUPPORT
            # HAS BEEN FOUND.
            # ----------------------------------------------------------

            # ----------------------------------------------------------
            # ----------------------------------------------------------
            # V16 SHARED-SUPPORT ACQUISITION
            # ----------------------------------------------------------
            #
            # Strong force feedback is used ONLY to FIND a valid
            # two-foot support state.
            #
            # As soon as loadOK=1:
            #
            #     load_cmd = 0
            #
            # and the measured shared-support posture is held.
            ...
            if self._recovery_load_ready:
                ...
                desired_left_load = float(
                    self._shared_support_left_load
                )
                ...
                if abs(load_error) <= deadband:
                    ...
                    load_cmd = float(
                        np.clip(
                            effective_error / 0.22,
                            -1.0,
                            1.0,
                        )
                    )

            self._recovery_desired_left_load = float(
                desired_left_load
            )

            self._recovery_load_cmd = float(
                load_cmd
            )

            # ----------------------------------------------------------
            # Only apply transfer while looking for shared support.
            # ----------------------------------------------------------

            if not self._recovery_load_ready:

                d = float(
                    self.shift_direction
                )

                # Strong transfer values already proven capable of
                # reaching shared support.
                target[7] += (
                    +0.105
                    * d
                    * load_cmd
            ...
            # ----------------------------------------------------------
            # V17 CLOSED-LOOP SHARED-SUPPORT COM HOLD
            # ----------------------------------------------------------
            #
            # Once shared support is found:
            #
            #     NO force-ratio bang-bang control
            #     NO neutral-centering
            #
            # Instead, regulate COM-y around the ACTUAL COM position
            # captured at the mechanically valid two-foot state.
            #
            # This restores the same lateral feedback structure that
            # was experimentally stable in V5.
            ...
            if (
                ...
                # Proven V5-style lateral PD law.
                com_hold_cmd = float(
                    np.clip(
                        3.00 * com_hold_error
                        + 0.45 * y_velocity_now,
                ...
                self._shared_hold_com_error = float(
                    com_hold_error
                )

                self._shared_hold_com_cmd = float(
                    com_hold_cmd
                )

                # IMPORTANT:
                # These are the same experimentally stable V5 signs.
                #
                # Apply AFTER the shared-support pose override so the
                # controller is not accidentally overwritten again.
            ...
            # AFTER loadOK:
            #
            # V16 applies NO additional force-transfer correction.
            #
            # The captured shared-support posture is held.
            # ----------------------------------------------------------
    ...
    def step(self, action=None):
        ...
        # --------------------------------------------------------------
        # V10B:
        # inspect the REAL physical state immediately after simulation.
        # --------------------------------------------------------------
        self._update_touchdown_state()

        # V12:
        # once touchdown exists, confirm stable TWO-FOOT load sharing.
        self._update_recovery_load_state()
    ...
    def _get_info(self) -> Dict[str, float]:
        ...
        return {
            ...
            "left_was_airborne": bool(
                self._left_was_airborne
            ),

            "touchdown_latched": bool(
                self._touchdown_latched
            ),

            "touchdown_step": float(
                self._touchdown_step
            ),

            "swing_confirmed": bool(
                self._swing_confirmed
            ),

            "left_air_streak": float(
                self._left_air_streak
            ),

            "left_max_air_streak": float(
                self._left_max_air_streak
            ),

            "touchdown_contact_streak": float(
                self._touchdown_contact_streak
            ),

            "left_peak_clearance": float(
                self._left_peak_clearance
            ),

            "recovery_load_streak": float(
                self._recovery_load_streak
            ),

            "recovery_load_ready": bool(
                self._recovery_load_ready
            ),

            "recovery_load_ready_step": float(
                self._recovery_load_ready_step
            ),

            "shared_support_com_y": float(
                self._shared_support_com_y
            ),

            "shared_support_left_load": float(
                self._shared_support_left_load
            ),

            "recovery_desired_left_load": float(
                self._recovery_desired_left_load
            ),

            "recovery_load_cmd": float(
                self._recovery_load_cmd
            ),

            "shared_hold_com_error": float(
                self._shared_hold_com_error
            ),

            "shared_hold_com_cmd": float(
                self._shared_hold_com_cmd
            ),
    ...
    def _compute_reward(
        ...
        return float(reward), {
            ...
            "reward_version": "deterministic_left_lift_v17_b2_peak_hold_extension",
        }
