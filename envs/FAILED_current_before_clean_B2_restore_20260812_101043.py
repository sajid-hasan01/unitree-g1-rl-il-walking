...
class G1DeterministicLeftLiftEnv(gym.Env):
    ...
    def _capture_initial_geometry(self) -> None:
        ...
        # V17-B3: preserve the initial sole orientation during swing.
        self.left_foot_R0 = np.asarray(
    ...
    def _ik_pose_delta(
        ...
        """
        V17-B3:
        Soft 6D swing-foot IK.

        Translation remains the primary task.

        Roll/pitch orientation toward the initial standing-foot
        orientation is phased in only during the upper part of swing.

        Yaw is intentionally weak because it is not required for
        vertical clearance.
        """
        ...
        #
        # cross(current_axis, desired_axis) has the correct corrective
        # sign for mj_jacSite's world-frame rotational Jacobian.
        rot_error = 0.5 * (
            np.cross(
                current_rotation[:, 0],
                desired_rotation[:, 0],
        ...
        # Conservative task weighting.
        #
        # x rotation -> foot roll
        # y rotation -> foot pitch
        # z rotation -> yaw
        #
        # Pitch/roll receive meaningful correction.
        # Yaw remains deliberately weak.
        orientation_weights = (
            orientation_phase
            * np.array(
                [0.12, 0.12, 0.025],
    ...
    def _target_joint_position(self, info: Dict[str, float]) -> np.ndarray:
        ...
        if (
            ...
            # V17-B3: fade sole-orientation control in only near peak swing.
            orientation_phase = self._smoothstep(
                (sw - 0.55) / 0.35
            )

            dq = self._ik_pose_delta(
                self.left_foot_site,
                swing_dofs,
                err,
                self.left_foot_R0,
