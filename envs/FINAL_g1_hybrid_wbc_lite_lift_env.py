...
class HybridLiftConfig:
    ...
    # V4 fixed high-level controller bias from empirical residual sweep.
    # Sweep result: stance_ankle=-1 removes backward root drift.
    # We use a moderated bias, plus reduced lift/swing amplitude, to avoid
    # the y_position_limit created by the full residual combo.
    bias_lift_amp: float = -0.95
    bias_lateral_shift: float = -0.35
    bias_torso: float = 0.00
    bias_stance_ankle: float = -0.85
    bias_stance_hip: float = 0.70
    bias_swing_shape: float = -0.95
...
class G1HybridWBCLiteLiftEnv(gym.Env):
    ...
    def _target_joint_position(self, action: np.ndarray, info: Dict[str, float]) -> np.ndarray:
        ...
        # V4: convert the empirical sweep result into controller structure.
    ...
    def _compute_reward(self, action: np.ndarray, info: Dict[str, float]) -> Tuple[float, Dict[str, float]]:
        ...
        return float(total), {
            ...
            "reward_version": "hybrid_wbc_lite_lift_v5_lateral_stabilized",
        }
