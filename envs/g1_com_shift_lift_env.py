...
class G1ComShiftLiftEnv(gym.Env):
    """
    Unitree G1 COM-shift + foot-lift curriculum environment. V5: less hip-pitch swing lift, stronger x anchor, and clearance control.
    ...
    def step(self, action):
        ...
        residual = self.action_scale * self.residual_scale * residual_mask * action
    ...
    def _lift_envelope(self) -> float:
        ...
        if self.stage_name in ("right_lift", "left_lift"):
            # No lift during pre-shift and final recovery phases.
            if phase < 0.45 or phase >= 0.82:
                return 0.0

            # Smooth lift up and down over a slower window.
    ...
    def _single_lift_active(self) -> bool:
        # Single-foot stages contain four phases:
        #   0.00-0.45: both feet down, COM shifts over support foot
        #   0.45-0.82: swing foot lifts and returns
        #   0.82-1.00: both feet down, balance recovers
        # This gives more time for COM shift and reduces sudden swing impulses.
        phase = self._phase01()
    ...
    def _target_y_offset(self) -> float:
        ...
        if self.stage_name in ("shift_left", "shift_right"):
        ...
        if self.stage_name in ("right_lift", "left_lift"):
            ...
            # Ramp into COM shift before foot lift, hold during lift, then recover.
            if phase < 0.42:
    ...
    def _residual_mask(self) -> np.ndarray:
        """
        During active single-foot swing, prevent PPO from cancelling the teacher
        on the swing leg. PPO can still stabilize using the support leg and waist.
        """
    ...
    def _teacher_offsets(self) -> np.ndarray:
        ...
        if left_swing:
            amp = scale * lift
            # Left swing leg lift.
            # Less hip pitch reduces backward root drift; knee/ankle create clearance.
            teacher[0] += 0.24 * amp
            teacher[1] += -0.14 * amp
            teacher[3] += 0.95 * amp
            teacher[4] += 0.50 * amp
    ...
    def _get_info(self) -> Dict[str, float]:
        ...
        info: Dict[str, float] = {
            ...
            "residual_mask_sum": float(np.sum(self._residual_mask())),
        }
        info.update(foot)
        return info
    ...
    def _compute_reward(self, action: np.ndarray, info: Dict[str, float]) -> Tuple[float, Dict[str, float]]:
        ...
        posture_reward = (
            3.0 * max(up_z, 0.0)
            + 1.3 * math.exp(-35.0 * (height - 0.79) ** 2)
        )
        ...
        if self.cfg.allow_x_motion:
            ...
            com_reward += 2.1 * math.exp(-95.0 * (x_vel - 0.0) ** 2)
            # Extra one-sided guard against the repeated backward-drift failure.
            com_reward -= 4.0 * max(0.0, -x_pos - 0.10)

        # 3. Foot phase.
        target_clearance = float(self.cfg.target_clearance)
        if left_swing:
            over = max(0.0, left_clear - target_clearance - 0.018)
            foot_phase_reward = 3.0 * math.exp(-420.0 * (left_clear - target_clearance) ** 2) - 8.0 * over
        elif right_swing:
            over = max(0.0, right_clear - target_clearance - 0.018)
            foot_phase_reward = 3.0 * math.exp(-420.0 * (right_clear - target_clearance) ** 2) - 8.0 * over
        else:
