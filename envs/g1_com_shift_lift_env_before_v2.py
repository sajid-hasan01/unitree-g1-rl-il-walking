...
class G1ComShiftLiftEnv(gym.Env):
    """
    Unitree G1 COM-shift + foot-lift curriculum environment.
    ...
    def _compute_reward(self, action: np.ndarray, info: Dict[str, float]) -> Tuple[float, Dict[str, float]]:
        ...
        com_reward = (
            1.8 * math.exp(-180.0 * (y_pos - y_target) ** 2)
            - 1.2 * abs(y_vel)
            - 0.8 * abs(x_pos)
        )
        if self.cfg.allow_x_motion:
            ...
            com_reward += 1.0 * math.exp(-35.0 * (x_vel - 0.0) ** 2)
    ...
    def _terminated(self, info: Dict[str, float]) -> bool:
        ...
        if abs(y_pos) > 0.50:
            return True
        if not self.cfg.allow_x_motion and abs(x_pos) > 0.38:
            return True
        if self.cfg.allow_x_motion:
            if x_pos > 0.25:
                return True
