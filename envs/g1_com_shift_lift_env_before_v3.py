...
class G1ComShiftLiftEnv(gym.Env):
    """
    Unitree G1 COM-shift + foot-lift curriculum environment. V2: safer shift_right/left_lift and stronger x anchoring.
    ...
    def _lift_envelope(self) -> float:
        phase = self._phase01()
        # Smooth lift during first half, lower during second half.
        if phase < 0.50:
    ...
    def _compute_reward(self, action: np.ndarray, info: Dict[str, float]) -> Tuple[float, Dict[str, float]]:
        ...
        if self.cfg.allow_x_motion:
            ...
            com_reward += 1.3 * math.exp(-55.0 * (x_vel - 0.0) ** 2)
    ...
    def _terminated(self, info: Dict[str, float]) -> bool:
        ...
        if abs(y_pos) > 0.50:
            return True
        if not self.cfg.allow_x_motion and abs(x_pos) > 0.42:
            return True
        if self.cfg.allow_x_motion:
            if x_pos > 0.25:
                return True
