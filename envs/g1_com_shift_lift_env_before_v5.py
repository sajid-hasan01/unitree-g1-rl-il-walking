...
class G1ComShiftLiftEnv(gym.Env):
    """
    Unitree G1 COM-shift + foot-lift curriculum environment. V4: slower lift, COM-shift ramp, and swing-leg residual lock.
    ...
    def _lift_envelope(self) -> float:
        ...
        if self.stage_name in ("right_lift", "left_lift"):
            ...
            if phase < 0.40 or phase >= 0.85:
                return 0.0

            # Smooth lift up and down.
            if phase < 0.72:
                local = (phase - 0.40) / 0.32
                if local < 0.50:
                    return smoothstep(local / 0.50)
                return smoothstep((1.0 - local) / 0.50)

            # Keep contact expectation as support during the put-down portion,
            # but allow teacher to gradually fade out instead of snapping.
            return smoothstep((0.85 - phase) / 0.13)
    ...
    def _single_lift_active(self) -> bool:
        ...
        #   0.00-0.40: both feet down, COM shifts over support foot
        #   0.40-0.72: swing foot lifts
        #   0.72-0.85: swing foot returns down
        #   0.85-1.00: both feet down, balance recovers
        # This gives more time for COM shift before the risky foot-lift phase.
        phase = self._phase01()
        return 0.40 <= phase < 0.72
    ...
    def _target_y_offset(self) -> float:
        ...
        if self.stage_name in ("right_lift", "left_lift"):
            ...
            if phase < 0.35:
                return raw * smoothstep(phase / 0.35)
            if phase < 0.78:
                return raw
            return raw * (1.0 - smoothstep((phase - 0.78) / 0.22))
