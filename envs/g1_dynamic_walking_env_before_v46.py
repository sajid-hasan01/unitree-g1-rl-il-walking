...
class G1DynamicWalkingEnv(gym.Env):
    ...
    def __init__(
        ...
        # jerks. For v45 clean walking, evaluate/train with 0.35 unless explicitly
    ...
    def _compute_reward(self, action):
        ...
        # v45 forward-drive shaping:
        # Earlier versions learned a stable stepping/rocking behavior but often
        # lost sustained negative-X progression. The old progress_reward was too
        # small because it was bounded by target_speed (~0.10), then multiplied by
        # only 0.20 in the final reward. Here we normalize progress to [0, 1] so
        # stable forward motion has a visible reward signal without rewarding
        # dangerous overspeed.
        if transition_alpha > 0.25 and target_speed > 1e-6:
            progress_reward = float(
                np.clip(directional_velocity / target_speed, 0.0, 1.0)
            )

            directional_position = direction_sign * float(self.data.qpos[0])
            forward_position_reward = float(
                np.clip(directional_position / 0.30, 0.0, 1.0)
            )
        else:
            progress_reward = 0.0
            forward_position_reward = 0.0
        ...
        if transition_alpha > 0.25:
            wrong_direction_penalty = (
                14.0 * wrong_direction_amount
                + 10.0 * (wrong_direction_amount ** 2)
            )

        # Penalize post-transition stalling, but do not force sprinting. This asks
        # the robot to keep moving in the intended direction after the gait has
        # blended in, while the existing overspeed and lateral penalties still
        # prevent the aggressive v44-style failure.
        forward_stall_penalty = 0.0
        if transition_alpha > 0.55 and target_speed > 1e-6:
            minimum_sustained_speed = max(0.045, 0.45 * target_speed)
            speed_shortfall = max(minimum_sustained_speed - directional_velocity, 0.0)
            forward_stall_penalty = (
                8.0 * speed_shortfall
                + 15.0 * (speed_shortfall ** 2)
            )
        ...
        reward = (
            ...
            + 2.20 * upright_reward
            + 1.50 * height_reward
            + 0.65 * progress_reward
            + 0.25 * forward_position_reward
    ...
    def step(self, action):
        ...
        info = {
            ...
            "directional_velocity": float(
                (1.0 if self.target_forward_velocity >= 0.0 else -1.0)
                * self.data.qvel[0]
            ),
