...
def diagnose(row, args):
    ...
    # --------------------------------------------------------------
    # Modern V17 physical swing checks.
    # --------------------------------------------------------------

    if not row["swing_confirmed"]:
    ...
    if not row["touchdown_latched"]:
        return "LEFT_TOUCHDOWN_NOT_CONFIRMED"

    if not row["shared_support_latched"]:
        return "TWO_FOOT_SUPPORT_NOT_CONFIRMED"

    if not row["landing_ok"]:
        return "LEFT_LANDING_FAILED"

    # The mechanical primitive can be successful while still being
    # slightly below the FINAL desired swing-height requirement.
    if row["main_clearance"] < args.strict_clearance:
        return "LEFT_CLEARANCE_BELOW_STRICT_TARGET"
...
def run_episode(args, viewer=None):
    ...
    while not done:
        ...
        # --------------------------------------------------------------
        # V17 physical airborne evaluation.
        #
        # The older evaluator only looked while the REQUESTED swing
        # clearance exceeded the strict threshold. That misses the real
        # airborne interval during the descending part of the swing.
        #
        # Evaluate the actual contact/force state instead.
        # --------------------------------------------------------------
        if (
            bool(info["lift_enabled"])
            and not bool(info.get("touchdown_latched", False))
        ):
            physical_air = (
                not bool(info["left_contact"])
                and float(info.get("left_normal_force", 0.0)) <= 1.0
            )
    ...
    # Physical evaluator measurement.
    air_steps = int(
        sum(air_flags)
    )

    measured_air_streak = int(
        max_consecutive_true(
            air_flags
        )
    )

    # V11+ controller independently tracks sustained force-free air.
    controller_air_streak = int(
        float(
            final.get(
                "left_max_air_streak",
                0.0,
    ...
    max_air_streak = max(
        measured_air_streak,
        controller_air_streak,
    )

    support_ok_ratio = (
        float(np.mean(support_flags))
        if support_flags
        else 0.0
    )
    ...
    row = {
        ...
        # Modern deterministic primitive state.
        "swing_confirmed": int(
            bool(
                final.get(
                    "swing_confirmed",
        ...
        "controller_peak_clearance": float(
            final.get(
                "left_peak_clearance",
                max_clear,
            )
...
def main():
            ...
            while not done and viewer.is_running():
                ...
                if env.episode_step % 25 == 0:
                    print(
                        ...
                        f"Cerr={info['shared_hold_com_error']:+.3f} "
                        f"Ccmd={info['shared_hold_com_cmd']:+.3f} "
    ...
    for ep in range(args.episodes):
        ...
        print(
            ...
            f"airOK={row['swing_confirmed']} "
            f"td={row['touchdown_latched']} "
            f"loadOK={row['shared_support_latched']} "
            f"pk={row['controller_peak_clearance']:.4f} "
    ...
    numeric_keys = [
        ...
        "air_steps",
        "max_air_streak",
        "controller_max_air_streak",
        "controller_peak_clearance",
        "swing_confirmed",
        "touchdown_latched",
