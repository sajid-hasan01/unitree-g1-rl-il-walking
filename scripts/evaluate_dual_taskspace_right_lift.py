...
def run_episode(args, action):
    ...
    while not done:
        ...
        left_clearances.append(float(info['left_foot_clearance']))
    ...
    return {
        ...
        'max_left_clearance': float(max(left_clearances)) if left_clearances else 0.0,
        'min_up_z': min_up,
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate dual-task-space right-foot lift with left support-foot lock.')
    parser.add_argument('--model_path', type=str, default='third_party/mujoco_menagerie/unitree_g1/scene.xml')
    parser.add_argument('--episodes', type=int, default=5)
    parser.add_argument('--csv', type=str, default='results/dual_taskspace_right_lift_eval.csv')
    ...
    print('=' * 118)
    print('DUAL TASK-SPACE RIGHT-FOOT LIFT EVALUATION')
    print('Fixed action:', action.tolist())
    print('Adds left support-foot lock to right-foot task-space lift.')
    print('=' * 118)
    ...
    for ep in range(args.episodes):
        ...
        print(
            ...
            f"L={row['left_contact']} R={row['right_contact']} strict={row['strict_pass']} reason={row['reason']}"
        )

    print('\nSUMMARY')
    for key in [
        'steps', 'reward', 'main_clearance', 'max_left_clearance', 'min_up_z', 'contact',
        'support_slip', 'air_steps', 'max_air_streak', 'support_ok_ratio', 'landing_ok',
