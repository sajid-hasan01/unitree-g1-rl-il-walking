...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Strict open-loop evaluator for the hybrid WBC-lite lift controller. "
            "Unlike the previous evaluator, this checks final lateral drift, y velocity, "
            "and expected landing/contact state."
        )
    )
    ...
    print("=" * 108)
    print("HYBRID WBC-LITE STRICT OPEN-LOOP EVALUATION V2")
    print("Fixed action:", action.tolist())
    print("Strict gates include x, y, y_velocity, uprightness, clearance, and expected landing/contact.")
    print("=" * 108)
    ...
    for ep in range(args.episodes):
        ...
        relaxed = int(
            ...
            and abs(float(final["y_position"])) <= 0.20
        )
        ...
        print(
            ...
            f"x={row['final_x']:+.3f} y={row['final_y']:+.3f}/{row['target_y']:+.3f} "
            ...
            f"land={row['expected_landing_ok']} strict={strict} relaxed={relaxed} reason={reason}"
        )
    ...
    print("\nSUMMARY")
    for key in [
        "steps", "reward", "main_clearance", "min_up_z", "contact", "slip",
        "final_x", "final_y", "target_y", "final_x_velocity", "final_y_velocity",
        "base_height", "root_ang_vel",
    ]:
