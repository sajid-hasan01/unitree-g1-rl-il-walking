...
def run_case(args, lift_height: float, ik_gain: float, ik_delta: float) -> dict:
    ...
    total_force = best_force["right_force_min"] + best_force["left_force_at_min"]
    right_ratio = best_force["right_force_min"] / max(total_force, 1e-6)
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv",
        default="experiments/amass_b3_15dof_residual_ppo_v3b/reports/wbc_v7_static_preload_ik_sweep.csv",
    )
    ...
    lift_values = [0.010, 0.015, 0.020]
    ik_gains = [0.55, 0.80, 1.05]
    ik_deltas = [0.060, 0.090]

    rows = []

    print("=" * 132)
    print("WBC V7 STATIC PRELOAD + FOOT IK SWEEP")
    print("Static preload fixed. Sweeping vertical foot IK authority.")
    print("=" * 132)

    for lift_height in lift_values:
        for ik_gain in ik_gains:
            for ik_delta in ik_deltas:
                row = run_case(args, lift_height, ik_gain, ik_delta)
                rows.append(row)
