...
def pass_stage(stage: str, metrics: Dict[str, float]) -> Tuple[bool, str]:
    ...
    final_x_abs = abs(metrics["final_x"])

    if stage in ("shift_left", "shift_right"):
        ok = steps >= 600 and up >= 0.90 and slip <= 0.12 and contact >= 0.90 and final_x_abs <= 0.25
        return ok, (
            "need steps>=600 up>=0.90 slip<=0.12 contact>=0.90 |final_x|<=0.25; "
            f"got steps={steps:.0f}, up={up:.3f}, slip={slip:.3f}, "
            f"contact={contact:.3f}, final_x={metrics['final_x']:.3f}"
        )

    if stage == "right_lift":
        ok = steps >= 450 and up >= 0.84 and max_r >= 0.025 and slip <= 0.24 and final_x_abs <= 0.35
        return ok, (
            "need steps>=450 up>=0.84 rightClr>=0.025 slip<=0.24 |final_x|<=0.35; "
            f"got steps={steps:.0f}, up={up:.3f}, rightClr={max_r:.3f}, "
            f"slip={slip:.3f}, final_x={metrics['final_x']:.3f}"
        )

    if stage == "left_lift":
        ok = steps >= 450 and up >= 0.84 and max_l >= 0.022 and slip <= 0.24 and final_x_abs <= 0.35
        return ok, (
            "need steps>=450 up>=0.84 leftClr>=0.022 slip<=0.24 |final_x|<=0.35; "
            f"got steps={steps:.0f}, up={up:.3f}, leftClr={max_l:.3f}, "
            f"slip={slip:.3f}, final_x={metrics['final_x']:.3f}"
        )
    ...
    if stage == "alt_lift":
        ...
        return ok, (
            "need steps>=500 up>=0.82 bothClr>=0.018 slip<=0.30; "
            f"got steps={steps:.0f}, up={up:.3f}, L/R={max_l:.3f}/{max_r:.3f}, slip={slip:.3f}"
        )

    if stage == "tiny_walk":
        ok = steps >= 450 and up >= 0.80 and max_l >= 0.016 and max_r >= 0.016 and slip <= 0.36
        return ok, (
            "need steps>=450 up>=0.80 bothClr>=0.016 slip<=0.36; "
            f"got steps={steps:.0f}, up={up:.3f}, L/R={max_l:.3f}/{max_r:.3f}, slip={slip:.3f}"
        )
...
def main():
    ...
    print("Important fix: clip_range and learning_rate are restored as SB3 schedules after loading.")
    print("=" * 100)
    ...
    for attempt in range(1, args.attempts + 1):
        ...
        # SB3 expects schedules/callables here after model creation/load.
        # Direct float assignment causes: TypeError: 'float' object is not callable.
