...
class QualityEvalCallback(BaseCallback):
    ...
    def _make_eval_env(self):
        return G1MimicPhaseLiftXLockEnv(
            ...
            no_lift_terminate_target=self.args.no_lift_terminate_target,
            no_lift_terminate_clearance=self.args.no_lift_terminate_clearance,
            no_lift_penalty_weight=self.args.no_lift_penalty_weight,
    ...
    def _run_eval_episode(self) -> Dict[str, float]:
        ...
        abs_final_xv = abs(float(final["x_velocity"]))
        contact_mean = float(np.mean(contacts)) if contacts else 0.0
        slip_mean = float(np.mean(slips)) if slips else 0.0

        # Strongly rejects both standing-still and forward-drift policies.
        clearance_bonus = 2600.0 * min(main_clearance, 0.045)
        no_lift_penalty = 180.0 if main_clearance < max(0.012, 0.50 * self.args.target_clearance) else 0.0
        score = (
            1.0 * steps
            + clearance_bonus
            + 140.0 * min_up
    ...
    def _on_step(self) -> bool:
        ...
        mean_xv = float(np.mean([r["final_x_velocity"] for r in rows]))
        reason = rows[0]["reason"]

        print(
            f"[XLOCK QUALITY] steps={mean_steps:.1f} clear={mean_clearance:.4f} "
            f"up={mean_up:.3f} x={mean_x:+.3f} xv={mean_xv:+.3f} "
            f"score={mean_score:.2f} reason={reason}"
        )

        eligible = (
            mean_clearance >= max(0.010, 0.60 * self.args.target_clearance)
            and abs(mean_x) <= self.args.x_hard_limit
            and mean_up >= 0.78
            and reason not in {"no_lift_mid_swing", "forward_x_limit", "xlock_position_limit"}
        )

        if eligible and mean_score > self.best_score:
            self.best_score = mean_score
            out = self.save_dir / "best_xlock_quality_model"
            self.model.save(str(out))
            print(f"[XLOCK QUALITY] New best xlock model saved: {out}.zip score={self.best_score:.2f}")
        elif not eligible:
            print("[XLOCK QUALITY] Not saved: failed clearance/root/stability gate.")

        return True


def main():
    parser = argparse.ArgumentParser(description="Train x-locked DeepMimic-lite phase lift policy.")
    ...
    parser.add_argument("--output", type=str, default="models/g1_mimic_phase_right_lift_xlock_v1.zip")
    parser.add_argument("--checkpoint_dir", type=str, default="models/g1_mimic_phase_right_lift_xlock_v1_checkpoints")
    parser.add_argument("--log_dir", type=str, default="logs/g1_mimic_phase_right_lift_xlock_v1")
    ...
    parser.add_argument("--clip_range", type=float, default=0.06)
    parser.add_argument("--ent_coef", type=float, default=0.004)
    parser.add_argument("--target_kl", type=float, default=0.018)
    ...
    parser.add_argument("--action_target_smoothing", type=float, default=0.50)

    parser.add_argument("--mimic_weight", type=float, default=0.12)
    parser.add_argument("--mimic_vel_weight", type=float, default=0.03)
    ...
    parser.add_argument("--x_hard_limit", type=float, default=0.22)
    ...
    print("=" * 100)
    print("XLOCK-V2 MIMIC-PHASE-LIFT TRAINING")
    print("Reference is reward-only; root-x exploit and zero-lift survival are hard-constrained.")
    print("stage:", args.stage)
    print("dataset:", args.dataset_path)
    print("reference_note:", getattr(probe, "reference_note", "unknown"))
    print("x_soft/x_hard:", args.x_soft_limit, args.x_hard_limit)
    print("=" * 100)
    ...
    callbacks = CallbackList(
        [
            CheckpointCallback(save_freq=args.checkpoint_freq, save_path=args.checkpoint_dir, name_prefix="g1_mimic_phase_xlock"),
            QualityEvalCallback(
                args=args,
                save_dir=str(Path(args.checkpoint_dir) / "xlock_quality_best"),
    ...
    model.learn(total_timesteps=args.total_timesteps, callback=callbacks, reset_num_timesteps=True, tb_log_name=f"mimic_phase_xlock_{args.stage}")
